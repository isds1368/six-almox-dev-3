...
def _aprovar_unificado():
    """
    Aba única com todos os pedidos pendentes — almoxarifado e compra —
    no mesmo layout visual, diferenciados por badge de tipo.
    """
    u = sessao()

    pend_alm  = listar_solicitacoes("pendente")
    pend_comp = listar_solicitacoes_compra("pendente")
    total     = len(pend_alm) + len(pend_comp)
    ...
    if pend_alm:
        st.markdown(
            '<div class="card"><div class="card-h">🏪 Almoxarifado — Pedidos Pendentes</div>',
            unsafe_allow_html=True,
        )
        for s in pend_alm:
            ...
            with c2:
                ...
                st.markdown(
                    f"<span style='font-size:.75rem;color:{cor};'>Disponível: {qtd_br(disp)} {un_lbl}</span>",
                    unsafe_allow_html=True,
                )
            with c3:
    ...
    if pend_comp:
        st.markdown(
            ...
            '<div class="card-h">🛒 Compras — Solicitações Pendentes</div>',
            unsafe_allow_html=True,
        )
        for s in pend_comp:
            ...
            with c2:
                ...
                st.markdown(
                    "<span style='font-size:.8rem;color:var(--info);'>🛒 Aguardando aprovação</span>",
                    unsafe_allow_html=True,
                )
            with c3:
