...
def _form_compra(u):
    if st.session_state.get("compra_enviada_ok"):
        st.success("📨 **Solicitação de Compra Enviada.** Você receberá retorno no aplicativo.")
    ...
    if st.button("📨 Enviar Solicitação de Compra →", type="primary", use_container_width=True, key="btn_sc"):
        erros = []
