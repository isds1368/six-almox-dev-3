    ...
    if enviou:
        ...
        if erros:
            ...
            criar_solicitacao_compra({
                ...
                # notificacao_compra_lida, codigo_requisicao, status_compra, entrega_confirmada
                # são gerenciados pelo banco (DEFAULT / trigger) — não enviar no INSERT
            })
    ...
    with cs:
        if st.button(f"{emoji} SIM, {titulo.lower()}", type="primary", use_container_width=True):
            ...
            if tipo == "compra":
                ...
                # Colunas adicionadas pela migration v2 — enviadas apenas se existirem no schema
                try:
                    extras = {"notificacao_compra_lida": False}
                    if acao == "aprovar":
                        extras["status_compra"] = STATUS_COMPRA_OPCOES[0]
                    atualizar_solicitacao_compra(conf["id"], {**dados, **extras})
                except Exception:
                    # Fallback: migration ainda não executada — salva só os campos base
