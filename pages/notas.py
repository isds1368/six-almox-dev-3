...
def _lista(movs, pendente: bool):
    ...
    for m in movs:
        ...
        with c1:
            ...
            if cnr:
                st.markdown(f"<span style='font-size:.74rem;color:var(--info);'>📋 CNR/Pedido: <strong>{cnr}</strong></span>",
                            unsafe_allow_html=True)
            st.markdown(f"<span style='font-size:.77rem;color:var(--t3);'>{prod.get('nome','—')}</span>",
                        unsafe_allow_html=True)
