...
def _form_manual():
    ...
    sn  = [s["nome"] for s in sets] or ["Sem setor"]
    SEM = "— não informar —"   # marcador para setores opcionais em branco

    # Tela de sucesso
    if st.session_state.get("saida_ok"):
        info = st.session_state["saida_ok"]
        linhas_setores = "".join(
            f'<div style="font-size:.8rem;color:var(--t3);">↳ {sv["setor"]}: '
            f'<strong>{qtd_br(sv["qtd"])} {sigla_para_opcao(info["un"])}</strong></div>'
            for sv in info["setores"]
        )
        st.markdown(
            ...
            f'<div style="font-size:1.2rem;font-weight:700;color:var(--ok);margin-bottom:.5rem;">Saída Registrada com Sucesso!</div>'
            f'<div style="font-size:.85rem;color:var(--t2);margin-bottom:.4rem;">'
            f'<strong>{qtd_br(info["qtd_total"])} {sigla_para_opcao(info["un"])}</strong>'
            f' de <strong>{info["produto"]}</strong></div>'
            f'{linhas_setores}'
            f'<div style="font-size:.8rem;color:var(--t3);margin-top:.4rem;">'
            f'Retirante: {info["retirante"]} &nbsp;|&nbsp; Por: <strong>{u.get("nick","")}</strong></div></div>',
    ...
    if st.session_state.get("confirmar_saida"):
        d = st.session_state["confirmar_saida"]; prod = d["prod"]
        un_lbl = sigla_para_opcao(d["ui"])
        linhas_conf = "".join(
            f"<br>&nbsp;&nbsp;↳ {sv['setor']}: {qtd_br(sv['qtd'])} {un_lbl}"
            for sv in d["setores"]
        )
        st.markdown(
            ...
            f'<div style="font-size:1rem;font-weight:700;color:var(--warn);margin-bottom:.8rem;">⚠️ Confirmar Saída</div>'
            f'<div style="font-size:.85rem;color:var(--t2);line-height:1.8;">'
            f'<b>Produto:</b> {esc(prod["nome"])}<br>'
            f'<b>Total:</b> {qtd_br(d["qtd_total"])} {un_lbl}<br>'
            f'<b>Setores:</b>{linhas_conf}<br>'
            f'<b>Retirante:</b> {d["nome_r"]}<br>'
            f'<b>Motivo:</b> {d["motivo"]}</div></div>',
            unsafe_allow_html=True)
        cs, cn, _ = st.columns([1,1,3])
        with cs:
            if st.button("✅ SIM, confirmar", type="primary", use_container_width=True):
                # Registra uma movimentação por setor para alimentar o dashboard corretamente
                ts = agora_iso()
                for sv in d["setores"]:
                    registrar_movimentacao({
                        ...
                        "tipo_saida":            "MANUAL",
                        "status":                "concluido",
                        "quantidade_informada":  sv["qtd"],
                        "unidade_informada":     d["ui"],
                        "quantidade_convertida": sv["qtd"],
                        "setor_solicitante":     sv["setor"],
    ...
    # Produto FORA do form para saldo em tempo real
    ...
    st.markdown(
        ...
        f'padding:.55rem .9rem;font-size:.79rem;margin:.3rem 0 .6rem;">'
        f'📦 Saldo disponível: <strong style="color:{cor};">{qtd_br(est)} {un_lbl}</strong></div>',
        unsafe_allow_html=True)

    ui = _u("Unidade", val=un_sec, key=f"man_ui_{prod['id']}")

    # ── Distribuição por setor (até 5) ───────────────────────────
    st.markdown(
        '<div style="font-size:.78rem;font-weight:700;color:var(--t3);'
        'letter-spacing:.05em;text-transform:uppercase;margin:.8rem 0 .2rem;">'
        'Distribuição por Setor</div>'
        '<div style="font-size:.75rem;color:var(--t3);margin-bottom:.5rem;">'
        '1º setor obrigatório — demais opcionais</div>',
        unsafe_allow_html=True)

    # Cabeçalho das colunas alinhado com as linhas abaixo
    _ch1, _ch2 = st.columns([3, 1])
    with _ch1: st.markdown('<div style="font-size:.75rem;color:var(--t3);padding-bottom:.1rem;">Setor</div>', unsafe_allow_html=True)
    with _ch2: st.markdown('<div style="font-size:.75rem;color:var(--t3);padding-bottom:.1rem;">Qtd</div>', unsafe_allow_html=True)

    linhas = []
    for i in range(5):
        c_s, c_q = st.columns([3, 1])
        with c_s:
            opts = sn if i == 0 else [SEM] + sn
            s_sel = st.selectbox(
                f"Setor {i+1}" + (" *" if i == 0 else " (opcional)"),
                opts, key=f"ms_s{i}",
                label_visibility="collapsed")
        with c_q:
            s_qtd = st.number_input(
                "Qtd", min_value=0.0,
                value=(1.0 if i == 0 else 0.0),
                step=1.0, key=f"ms_q{i}",
                label_visibility="collapsed")
        linhas.append((s_sel, s_qtd))
    ...
    if st.button("Registrar Saída →", type="primary", use_container_width=True, key="btn_man_saida"):
        setores_validos = []
        for i, (s_sel, s_qtd) in enumerate(linhas):
            preenchido = (i == 0) or (s_sel != SEM)
            if preenchido and s_qtd > 0:
                setores_validos.append({"setor": s_sel, "qtd": s_qtd})

        qtd_total = sum(sv["qtd"] for sv in setores_validos)
        erros = []
        if not nome_r.strip():  erros.append("Nome obrigatório.")
        if not motivo.strip():  erros.append("Motivo obrigatório.")
        if not setores_validos: erros.append("Informe pelo menos um setor com quantidade maior que zero.")
        if est < qtd_total:     erros.append(f"Estoque insuficiente. Disponível: {qtd_br(est)} {un_lbl}.")
        if erros:
            for e in erros: st.error(e)
        else:
...
def _tbl(movs):
    ...
    for m in movs:
        ...
        motivo = motivo[:40] + ("…" if len(motivo) > 40 else "")
        rows += (f'<tr><td style="color:var(--t3);font-size:.73rem;">{datahora_br(m["criado_em"])}</td>'
                 f'<td><strong>{esc(prod.get("nome","—"))}</strong></td>'
                 f'<td>{qtd_br(m["quantidade_informada"])} {un_lbl}</td>'
                 f'<td>{esc(m.get("setor_solicitante","—"))}</td>'
