"""
pages/controle.py — Mapa 2D do Almoxarifado com rastreabilidade de equipamentos
--------------------------------------------------------------------------------
Página chamada pelo app.py através do botão "Controle" no menu principal
(veja a rota "controle" em app.py). Expõe a função tela_controle(), que
desenha o mapa e os painéis de recebimento/alocação/quebra/troca.

Fluxo de vida do equipamento:
...
(status, setor, data/hora), desenhado no mapa como um caminho (linha +
marcadores numerados) — essa é a base da rastreabilidade de vida útil.

Persistência: por enquanto usa st.session_state (some ao reiniciar o app).
Os pontos de extensão para trocar por Supabase estão marcados com TODO em
registrar_evento() e carregar_estado() — sugestão de schema: tabela
equipamentos (id, nome, status, setor_atual) e equipamentos_historico
(id, equipamento_id, status, setor, data).
"""
