...
def _form_entrada(prod, u, cm):
    ...
    with st.form("fer"):
        ...
        pdf = None
        if te == "Nota Fiscal":
            pdf = st.file_uploader("📎 PDF da Nota Fiscal (necessário para gerar alerta de envio)",
                                   type=["pdf","png","jpg"])
        else:
            pdf = st.file_uploader("📎 Anexar comprovante (opcional)", type=["pdf","png","jpg"])
        ...
        if st.form_submit_button("✅ Confirmar Entrada", type="primary", use_container_width=True):
            ...
            if erros:
                ...
                if pdf:
                    ...
                    doc = criar_documento({"nome_arquivo": pdf.name, "caminho_arquivo": url,
                                           "status_envio": "pendente" if te=="Nota Fiscal" else "nao_requer"})
                    did = doc["id"]

                gerar = te == "Nota Fiscal" and pdf is not None

                # Inclui CNR na observação para facilitar rastreio
                obs_final = obs.strip()
                if cnr.strip():
                ...
                elif te == "Nota Fiscal" and not pdf:
                    st.warning("⚠️ Entrada NF sem PDF — nenhuma pendência criada em Notas Fiscais.")
...
def _reabastecimento():
    ...
    with st.form("freab"):
        ...
        pdf = None
        if te == "Nota Fiscal":
            pdf = st.file_uploader("📎 PDF da Nota Fiscal", type=["pdf","png","jpg"])
        else:
            pdf = st.file_uploader("📎 Anexar comprovante (opcional)", type=["pdf","png","jpg"])
