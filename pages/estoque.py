...
def _fmt_valor(v):
    """Formata um float como moeda BR: R$ 1.234,56"""
    try: v=float(v or 0)
    except (TypeError,ValueError): v=0.0
...
def _inv():
    ...
    for p in fil_pag:
        ...
        valor_estoque=float(p.get("valor_unitario") or 0)*est
        valor_html=f'<span style="font-weight:600;color:var(--t2);">{_fmt_valor(valor_estoque)}</span>'
        rows+=f'<tr><td><strong>{p["nome"]}</strong> {sit_html}</td><td class="mono">{p["codigo_interno"]}</td><td class="mono" style="color:var(--t4);">{p.get("ean") or "—"}</td><td style="color:var(--t3);">{cat}</td><td><strong>{qtd_br(est)} {us_lbl}</strong><br><span style="font-size:.71rem;color:var(--t3);">= {qtd_br(estp)} {up_lbl}</span>{res_html}</td><td>{valor_html}</td><td style="color:var(--t3);">{qtd_br(minp)} {up_lbl}</td><td>{badge(txt,cls)}</td></tr>'
    vz='<tr><td colspan="8" style="text-align:center;color:var(--t3);padding:2rem;">Nenhum resultado</td></tr>'
    st.markdown(f'<table class="tbl"><thead><tr><th>Produto</th><th>Código</th><th>EAN</th><th>Categoria</th><th>Estoque</th><th>Valor em Estoque</th><th>Mínimo</th><th>Status</th></tr></thead><tbody>{rows or vz}</tbody></table>',unsafe_allow_html=True)
...
def _editar():
    ...
    with st.form("fep"):
        ...
        with c3:
            rce=st.selectbox("Reposição contínua?",["Não","Sim"],
                              index=1 if p.get("reposicao_continua") else 0,key="rce")=="Sim"
        with c4:
            iee=st.selectbox("Insumo estratégico?",["Não","Sim"],
                              index=1 if p.get("insumo_estrategico") else 0,key="iee")=="Sim"
        with c5:
            vue=st.text_input("Valor unitário (R$)",value=_valor_editavel(p.get("valor_unitario")),key="vue")
        de=st.text_area("Descrição",value=p.get("descricao") or "")
        if st.form_submit_button("Salvar →",type="primary"):
            valor_e=_parse_valor(vue)
            if valor_e is None:
                st.error("Valor unitário inválido.")
            else:
                atualizar_produto(p["id"],{"nome":ne.strip(),"categoria_id":cm.get(ce),"unidade_primaria":upe,"unidade_secundaria":use,"fator_conversao":fe,"estoque_minimo_primario":eme,"ean":eane.strip() or None,"descricao":de.strip() or None,"ativo":ate,"reposicao_continua":rce,"insumo_estrategico":iee,"valor_unitario":valor_e})
